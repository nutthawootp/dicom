...
class PatientInfo:
    ...
    StudyDate: str = 'Not Found / Empty'  # เพิ่มตัวแปร Study Date
...
class DICOMScanner:
    ...
    def _extract_patient_info(dataset: pydicom.Dataset) -> PatientInfo:
        ...
        return PatientInfo(
            ...
            StudyDate=processor.get_attribute_safe(dataset, 'StudyDate', 'Not Found / Empty'), # ดึงค่า Study Date
...
class DataPreviewDialog(BaseDialog):
    def __init__(self, parent, patient_info: PatientInfo, series_info: Dict[str, SeriesData]):
        super().__init__(parent, "ตรวจสอบข้อมูล DICOM ต้นฉบับ", "600x700") # ขยายหน้าต่างรองรับ Study Date
    ...
    def _create_patient_info_frame(self):
        ...
        tags_display = [
            ...
            ("(0008,0020) Study Date", self.patient_info.StudyDate), # แสดงผล Study Date ในหน้า Preview
...
class FinalResultDialog(BaseDialog):
    def __init__(self, parent, result: ProcessResult, output_dir: str, original_info: PatientInfo, new_subject_id: str, new_protocol: str):
        super().__init__(parent, "กระบวนการเสร็จสมบูรณ์ (Process Completed)", "750x730") # ขยายเพื่อรับข้อมูลเพิ่ม
        self.result = result
    ...
    def _build_ui(self):
        ...
        tree = ttk.Treeview(table_frame, columns=columns, show="headings", height=11)
        ...
        comparison_data = [
            ...
            ("Study Date", self.original_info.StudyDate, f"{self.original_info.StudyDate} (คงเดิม)"), # เพิ่ม Study Date โดยแสดงสถานะ (คงเดิม) 
